...
function validatePathQuery(filePath, extensions) {
  ...
  if (/index$/.test(filePath)) {
    throw new Error("PageCreator: To query node \"path\" the \"filePath\" argument must omit index.\nPlease change this to: \"" + filePath.replace(/index$/, "") + "\"");
  }
  ...
  if (file.length === 0 || file[0].length === 0) {
    throw new Error("PageCreator: To query node \"path\" the \"filePath\" argument must represent a file that exists.\nUnable to find a file at: \"" + absolutePath + "\"");
  }
}