...
const getReadmeForPlugin = async name => {
  try {
    return fetch(`https://unpkg.com/${name}/README.md`).then(res => res.text()).catch(() => null);
  } catch (err) {
    return null;
  }
};
...
const read = async ({
  ...
  try {
    ...
    if (plugin) {
      const [description, readme] = await Promise.all([getDescriptionForPlugin(id), getReadmeForPlugin(id)]);
      const {
        shadowedFiles,
        shadowableFiles
