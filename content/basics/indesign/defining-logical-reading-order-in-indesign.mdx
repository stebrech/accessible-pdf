...
---
<NoteMessage>
What does <Link to='/glossary/#logical-reading-order'>logical reading order</Link> mean? Find out more in the glossary.
</NoteMessage>
...
<NoteMessage>
More about <ExternalLink linktext={`threading text`} link={`https://helpx.adobe.com/indesign/using/threading-text.html`}/> you can read in Adobe’s online manual.
</NoteMessage>
...
<NoteMessage>
More about <ExternalLink linktext={`anchored objects`} link={`https://helpx.adobe.com/indesign/using/anchored-objects.html`}/> you can read in Adobe’s online manual.
</NoteMessage>

<NoteMessage>
Since images are non-text content, an alternative text must be provided if they are relevant to the content. If they are not relevant, they do not belong in an article. For further information, see also <Link to='/glossary/#alt-text)'>alt text</Link> in the glossary or [“Add an alt text”](https://accessible-pdf.info/en/basics/add-an-alt-text/).
</NoteMessage>
